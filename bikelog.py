...
def log_ride(date, name, distance, bike):
    ...
    # Print year-to-date mileage per rider for the current year
    current_year_file = get_csv_filename(datetime.now().year)
    totals = {}
    if os.path.exists(current_year_file):
        with open(current_year_file, 'r', newline='') as f:
            reader = csv.DictReader(f)
            for row in reader:
                rider = row['Name']
                try:
                    totals[rider] = totals.get(rider, 0.0) + float(row['Distance'])
                except ValueError:
                    pass
    print(f"\n  {datetime.now().year} mileage by rider:")
    for rider, miles in sorted(totals.items(), key=lambda x: x[1], reverse=True):
        print(f"    {rider}: {miles:.2f} miles")
