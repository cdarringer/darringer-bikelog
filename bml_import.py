...
LEGACY_RECORDS = [
    ...
    {'date': '2025-05-26', 'bike': 'Cannondale ST400',
     ...
     'cost': 54.44, 'shop': 'Bicycles NYC', 'mileage': None},
    ...
    {'date': '2025-04-06', 'bike': 'Cannondale M700',
     ...
     'cost': 43.55, 'shop': 'Bicycles NYC', 'mileage': None},
    {'date': '2024-12-07', 'bike': 'Cannondale M700', 'activity': 'New Sunlite water bottle cages',
     'cost': 21.76, 'shop': 'Amazon.com', 'mileage': 1302},
...
def main():
    ...
    rows = []
    for i, r in enumerate(LEGACY_RECORDS, 1):
        date_obj = datetime.strptime(r['date'], '%Y-%m-%d')
        date_str = date_obj.strftime('%-m/%-d/%Y')

        if r['mileage'] is None:
            mileage = get_bike_mileage(r['bike'], date_obj)
            mileage_str = f'{mileage:.1f}'
            print(f"  [{i:2d}] {r['date']} {r['bike']:<22} mileage calculated: {mileage:,.1f}")
        else:
            mileage_str = f"{float(r['mileage']):.1f}"
            print(f"  [{i:2d}] {r['date']} {r['bike']:<22} mileage from source: {r['mileage']:,.0f}")

        rows.append([date_str, r['bike'], r['activity'], f"{r['cost']:.2f}", r['shop'], mileage_str])
    ...
    print(f"\nWrote {len(rows)} records to {MAINTENANCE_CSV}")
